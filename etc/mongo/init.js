...
for (let i = 0; i < N; ++i) {
	...
	newQuestion.author = DBRef(USER_COLLECTION_NAME, allUsers[randomInt(0, allUsers.length)]._id)
...
allQuestions.forEach(function(question, i, arr) {
	for (let j = 0; j < N; ++j) {
		...
		newAnswer.author = DBRef(USER_COLLECTION_NAME, allUsers[randomInt(0, allUsers.length)]._id)
...
for (let i = 0; i < N; ++i) {
	newSubscription = {}
	newSubscription.subscriber = DBRef(USER_COLLECTION_NAME, allUsers[randomInt(0, allUsers.length)]._id)
...
allQuestions.forEach(function(question, i, arr) {
	...
	for (let j = 0; j < subscriptionAmountForCurrentQuestion; ++j) {
		...
		db.getCollection(SUBSCRIPTION_COLLECTION_NAME).findAndModify({
			query : {subscriber: userForSubscribe},
			update : {$addToSet : {subscriptions : DBRef(QUESTION_COLLECTION_NAME, question._id)}}
		})
	}
})
