...
public class ElasticSearchServiceImpl implements ElasticSearchService {
    ...
    public String getAllQuestions() throws IOException {
        final ImmutableMap<String, String> params = new ImmutableMap.Builder<String, String>()
                .put("pretty", "true")
                .build();
        String queryMatchAll = "{\"query\": { \"match_all\": {} }}";

        return EntityUtils.toString(
                restClient.performRequest(
                        "GET", String.format("/%s/%s/_search", index, Question.QUESTION_COLLECTION_NAME), params,
                        new NStringEntity(queryMatchAll, ContentType.APPLICATION_JSON)
                ).getEntity());
    }
